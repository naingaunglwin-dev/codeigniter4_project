<?php
    ...
    class JobPostDetailModel extends Model{
        ...
        public function post_detail($id) {
            ...
            $this->post_detail->select('job_post.*, location.name As location_name, industry.name As industry_name, user.description As user_desc')
                              ->join('location', 'job_post.location = location.id', 'left')
                              ->join('industry', 'job_post.industry = industry.id', 'left')
                              ->join('user', 'job_post.user_id = user.id', 'left')
